...
- **reveal-or-die's parallel e2e run has started flaking again, one test per run**, which matters because `HANDOFF.md` records the opposite ("e2e used to fail in parallel and no longer does", fixed by the `getCellsInZones` index). Measured across three consecutive parallel runs on an unchanged tree: one failure each, a DIFFERENT test each time (`layout-shell` on a `page.goto` `ERR_ABORTED`, `contracts` write-function on the contract selector reading `AvatarsSale_Proxy` instead of `Game`), and the failing test passes when run alone. A serial `CI=1` run is green at 49, with one retried. Three different tests failing once each is a resource symptom rather than three bugs, and the handoff's own lesson applies unchanged: **a failure whose only symptom is under parallel load is a resource problem until proven otherwise**, and the tempting fixes all hide it. It is NOT this phase's, it did not arrive with this phase's change (which moves an import path and nothing else), and it should be measured from outside the suite the way the last one was rather than reasoned about. The template's own parallel run is 50 of 50 green, so whatever it is, it is not shared.
