...
export function createBalanceStore(
	...
	const {publicClient, signer, costs} = params;
	...
	async function fetchState($signer: Signer, $costs: Costs): Promise<boolean> {
		...
		set({
			step: 'Loaded',
			value: balance,
			credits: Number((balance * 100n) / $costs.creditsDivider) / 100,
		});
		return true;
	}
	...
	async function fetchNow(signer: Signer) {
		...
		try {
			const success = await fetchState(signer, $costs);
			if (!success) {
				interval = 500;
			}
		} finally {
	...
	function start() {
		...
		unsubscribeFromCost = costs.subscribe(async (newCosts) => {
			$costs = newCosts;
			if ($signer) {
				fetchNow($signer);
			}
