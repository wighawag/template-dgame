...
{#if !$splash.complete}
	{#if $splash.stage >= 1 || !browser}
		...
		<div class="overlay game-logo" out:fade|global on:click={() => splash.nextStage()}>
			<div class="content">
				<img
					src={url('/game-title.png')}
					alt="reveal-or-dire Logo Title"
				...
				<div class="loading-bar-container">
					<div
						class="loading-bar"
						style="width: {Math.max(0, Math.min(100, $splash.loadingValue * 100))}%"
					></div>
				</div>
	...
	{:else}
		<!-- svelte-ignore a11y-click-events-have-key-events -->
		<!-- svelte-ignore a11y-no-static-element-interactions -->
		<div class="overlay studio-logo" out:fade|global on:click={() => splash.nextStage()}>
			<div class="content">
				<img
