...
export function createSplashStore(loadingStore?: Readable<number> | undefined) {
	...
	let $data: SplashState = { stage: visited ? 1 : 0, loadingValue: 0, complete: false };
	const store = writable<SplashState>($data);

	function set(data: Omit<SplashState, 'complete'>) {
		$data = { ...data, complete: data.stage === MAX_STAGE && data.loadingValue >= 1 };
		store.set($data);
	}
	...
	function _loaded(timeIn: number) {
		...
		if (diff > timeIn) {
			...
			if (!params['logo']) {
				timeout = setTimeout(() => nextStage(), timeIn - diff) as unknown as number;
			}
		}
	}
