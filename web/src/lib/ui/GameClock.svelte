<script lang="ts">
	...
	const {twoPhase, epochInfo, localState, deployments} = getUserContext();
	...
	let percentage = $derived(
		$twoPhase.type === 'timed'
			? Math.min(
					...
					Math.max(
						0,
						100 - Math.floor(($twoPhase.timeLeft * 100) / $twoPhase.duration),
					),
				)
			: 0,
	);
	...
	let background = $derived(
		$twoPhase.phase == 'play' ? 'oklch(79.2% 0.209 151.711)' : 'red',
	);
...
{#if $twoPhase.type === 'timed'}
	...
	{#if $twoPhase.phase === 'wait' && $twoPhase.timeLeft > 0.1 && !!$localState.signer}
		<div
