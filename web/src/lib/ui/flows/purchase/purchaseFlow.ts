...
export function createPurchaseFlow(params: {
	writes: WriteOperations;
	avatars: AvatarsCollectionStore;
}) {
	const {writes, avatars} = params;
	...
	async function purchase() {
		try {
			...
			const hasAvatar = () =>
				$avatars.step === 'Loaded' &&
				$avatars.avatarsOnBench.find((v) => v == avatarID);
			while (!hasAvatar()) {
...
export type PurchaseFlowStore = ReturnType<typeof createPurchaseFlow>;
